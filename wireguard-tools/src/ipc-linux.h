...
static int parse_learnedips(const struct nlattr *attr, void *data)
{
	struct wgpeer *peer = data;
	struct wgallowedip *new_allowedip = calloc(1, sizeof(*new_allowedip));
	int ret;
	...
	if (!peer->first_learnedip)
		peer->first_learnedip = peer->last_learnedip = new_allowedip;
	else {
...
static int parse_peer(const struct nlattr *attr, void *data)
{
	...
	case WGPEER_A_LEARNEDIPS:
		return mnl_attr_parse_nested(attr, parse_learnedips, peer);
...
static void coalesce_peers(struct wgdevice *device)
{
	...
	while (peer && peer->next_peer) {
		...
		if (!peer->first_learnedip) {
			peer->first_learnedip = peer->next_peer->first_learnedip;
			peer->last_learnedip = peer->next_peer->last_learnedip;
		} else if (peer->next_peer->first_learnedip) {
			peer->last_learnedip->next_allowedip = peer->next_peer->first_learnedip;
			peer->last_learnedip = peer->next_peer->last_learnedip;
		}
