...
struct wgpeer {
	...
	struct wgallowedip *first_learnedip, *last_learnedip;
...
static inline void free_wgdevice(struct wgdevice *dev)
{
	...
	for (struct wgpeer *peer = dev->first_peer, *np = peer ? peer->next_peer : NULL; peer; peer = np, np = peer ? peer->next_peer : NULL) {
			...
			for (struct wgallowedip *allowedip = peer->first_learnedip, *na = allowedip ? allowedip->next_allowedip : NULL; allowedip; allowedip = na, na = allowedip ? allowedip->next_allowedip : NULL) {
				free(allowedip);
			}
		free(peer);
	}
