...
get_peer(struct wg_peer *peer, struct sk_buff *skb, struct dump_ctx *ctx)
{
	...
	if (!allowedips_node) {
		...
		learnableips_node =
			list_first_entry_or_null(&peer->learnableips_list,
					struct allowedips_node, peer_list);
	}
	...
	if (learnableips_node) {
		...
		list_for_each_entry_from(learnableips_node, &peer->learnableips_list,
					 peer_list) {
			u8 cidr, ip[16] __aligned(__alignof(u64));
			int family;
...
static int set_peer(struct wg_device *wg, struct nlattr **attrs)
{
	...
	if (attrs[WGPEER_A_LEARNABLEIPS]) {
		...
		nla_for_each_nested(attr, attrs[WGPEER_A_LEARNABLEIPS], rem) {
			...
			ret = set_allowedip(peer, learnableip, &peer->learnable_ips, &peer->learnableips_list);
			if (ret < 0)
				goto out;
		}
	}
